...
function pcBossPhaseEvolution(){
 ...
 if(level>1){
   const p=new THREE.Vector3(0,.2,EZ);pcEvolutionImpact(boss||enemyCards.find(Boolean),p,level===3?2:1.45);
...
async function pcUltimateCinematic(side,i,c,attackName='ULTIMATE NÉMESIS'){
 ...
 if(g){pcEvolutionImpact(c,g.position,2.1);v1892ScreenShake()}sfx('boss');await wait(720);d.classList.add('strike');await wait(520);d.classList.add('out');await wait(160);d.remove()
}
