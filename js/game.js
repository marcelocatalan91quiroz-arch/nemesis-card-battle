...
async function applyExternalMagic(side,c){
 ...
 if(dmIs(c)&&['DM-013','DM-014','DM-015'].includes(c.id)){
  ...
  nemesisEquip(side,idx,c.id==='DM-014'?'relic':'weapon',c,{atkBonus:Number(d.atk_bonus||0),defBonus:Number(d.def_bonus||0),flag:'_dm_'+c.id.replace('-','_')});
  if(c.id==='DM-014'){own.forEach(x=>{if(dmIs(x)&&x.type==='monster'){x.atk+=1000;x.def+=1000;x._dmTorch=true}})}
  ...
  toast(c.name+' equipada y sinergia Duel Master activada.');return true;
 }
