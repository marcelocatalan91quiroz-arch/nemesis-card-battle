...
function menuScene(){
 ...
 <div class="menu-panel"><h2>MAZO DE BATALLA</h2><p>Elige hasta <b>11 cartas</b> de tu colección para usar en batalla.</p><div class="mini-deck">${state.deck.slice(0,6).map(id=>{const c=card(id);return c?`<img src="${c.img}" loading="lazy" decoding="async" fetchpriority="low" alt="${esc(c.name)}" title="${esc(c.name)}">`:''}).join('')}${state.deck.length>6?`<span class="mini-deck-more">+${state.deck.length-6} CARTAS</span>`:''}</div><b>${state.deck.length}/11 seleccionadas</b></div>
