...
function nemesisEquip(side,i,kind,sourceCard,opts={}){
 ...
 slots[kind]=meta;
 pcEquipVisual(board?.[side]?.[i],kind,meta.label,meta.temporary);pcEquipmentCinematic(side,i,kind,meta);
...
let pcIntroPlayed=false,pcIntroTimers=[];
function pcStopIntroInstrumental(){pcIntroTimers.forEach(clearTimeout);pcIntroTimers=[]}
function pcIntroInstrumental(){
 if(pcIntroPlayed)return;pcIntroPlayed=true;pcStopIntroInstrumental();
 const melody=[220,277.18,329.63,440,392,329.63,293.66,220,246.94,329.63,369.99,493.88,440,369.99,329.63,277.18];
 const bass=[55,55,65.41,73.42,55,82.41,73.42,65.41];
 melody.forEach((n,i)=>pcIntroTimers.push(setTimeout(()=>{pcTone(n,.62,i%4===3?'sawtooth':'triangle','music',.025);if(i%2===0)pcTone(n*2,.28,'sine','music',.009)},i*410)));
 bass.forEach((n,i)=>pcIntroTimers.push(setTimeout(()=>pcTone(n,1.1,'triangle','music',.026),i*820)));
 pcIntroTimers.push(setTimeout(()=>{pcStopIntroInstrumental();pcStartBossMusic()},6900));
}
function pcStartBossMusic(){clearInterval(pcMusicTimer);const notes=isHades?[73,110,98,82]:isAres?[82,123,92,138]:isGhostGod?[92,138,185,123]:isSpectralKing?[87,130,174,116]:isSoulKnight?[82,110,147,98]:isRa?[110,165,220,147]:isDragon?[82,123,164,110]:[98,147,196,131];let n=0;pcTone(notes[0],1.8,'triangle','music',.035);pcMusicTimer=setInterval(()=>pcTone(notes[n++%notes.length],1.65,isRa?'sine':'triangle','music',.035),1450)}
function sfx(kind){const f={place:170,flip:260,attack:95,impact:55,fusion:320,destroy:62,fire:115,thunder:48,roar:74,equip:410,magic:520,trap:66,transform:245,ultimate:42,grave:78,phase:58,ice:620,shadow:92,solar:740,divine:880}[kind]||140;const wave={thunder:'square',trap:'sawtooth',ultimate:'sawtooth',shadow:'triangle',divine:'sine'}[kind]||(kind==='impact'||kind==='destroy'?'sawtooth':'triangle');pcTone(f,kind==='roar'||kind==='ultimate'?.55:.22,wave,'fx',kind==='impact'||kind==='ultimate'?.12:.075)}
function pcElementSfx(element){sfx(({FIRE:'fire',BLOOD:'fire',LIGHTNING:'thunder',ICE:'ice',DARK:'shadow',COSMIC:'shadow',LIGHT:'solar'})[element]||'impact')}
document.addEventListener('pointerdown',()=>{const a=pcAudioInit();a?.ctx?.resume?.();pcIntroInstrumental()},{once:true});
...
function dmPay(c,n){const st=dmState(c);if(st.energy<n){toast(c.name+': Energía insuficiente ('+st.energy+'/'+n+').');return false}st.energy-=n;return true}
...
function nemesisDmTurnStart(){
 ...
 playerGrave.forEach(c=>{if(dmIs(c,'DM-010'))dmState(c).austral++});nemesisDmSync()
}
...
function nemesisDmKeepTurnAfterAttack(c){
 ...
 if(c.id==='DM-004'&&c._dmSecondAttackTurn===turnNo&&c._dmSecondAttackUsedTurn!==turnNo){c._dmSecondAttackUsedTurn=turnNo;return true}
 return false
}
...
async function resolveBattle(attSide,ai,defSide,di){
 ...
 if(diff>0){
  ...
  toast(`¡Muro impenetrable! El atacante rebota y pierde ${rebote} HP.`);
 }else{
...
async function idrTryTransform(c){
 ...
 try{
  sfx('transform');await pcTransformationCinematic(c,to,side,i);
...
nemesisBossTurnStart();endPlayerMagicTurn();v172ClosePicker();v171HideAttackConfirm();v17PendingTarget=-1;v181ReturnOverview();v12TargetCamera(false);targetbanner?.classList.add('hidden');
 ...
 try{
  ...
  if(attackers.length&&!targets.length){
   ...
   await guardStep(revealEnemy(enemySlot),2200,'revelación rival');setPhase('ENEMY',`TURNO ${turnNo} DE ${enemyTurnName} · HABILIDAD`);await guardStep(useCreatureSkill('e',enemySlot),1800,'habilidad rival');if(phpv<=0)return finish(false);
...
const PC_BOSS_PHASE_VISUALS={
 ares:{1:{label:'ESTRATEGA',color:0xff7a2f},2:{label:'DIOS DE LA GUERRA',color:0xff2d1c},3:{label:'BESTIA DIVINA',color:0xc90000}},
 hades:{1:{label:'REY DEL INFRAMUNDO',color:0x7c39c9},2:{label:'TÁRTARO ABIERTO',color:0x441078},3:{label:'SEÑOR DE LAS ALMAS',color:0x1a062e}},
 ra:{1:{label:'IRA SOLAR',color:0xffc53d},2:{label:'DISCO DE RA',color:0xff8c19},3:{label:'SOL ANCESTRAL',color:0xff4c00}},
 'dios-fantasma':{1:{label:'SERAFÍN ESPECTRAL',color:0xa881ff},2:{label:'ESENCIA CELESTIAL',color:0x6b49d9},3:{label:'DIOS FANTASMA',color:0xe5d6ff}}
};
let pcBossVisualPhase=0;
function pcBossVisualLevel(){
 const ratio=enemyMaxHp?ehpv/enemyMaxHp:1;
 if(isAres)return aresPhase();
 return ratio>.66?1:ratio>.33?2:3
}
function pcBossPhaseEvolution(){
 const cfg=PC_BOSS_PHASE_VISUALS[duelKey];if(!cfg)return;
 const level=pcBossVisualLevel();if(level===pcBossVisualPhase)return;pcBossVisualPhase=level;
 const v=cfg[level]||cfg[1],battleEl=document.querySelector('.battle');
 battleEl?.setAttribute('data-boss-visual-phase',String(level));
 battleEl?.style.setProperty('--boss-phase-color','#'+v.color.toString(16).padStart(6,'0'));
 const badge=document.getElementById('bossPhaseBadge');
 if(badge&&!isAres){badge.innerHTML=`<small>${enemyDisplayName} · FASE ${level}</small><b>${v.label}</b><span>ARENA EN EVOLUCIÓN</span>`}
 if(level>1){
   sfx('phase');const p=new THREE.Vector3(0,.2,EZ);pcEvolutionImpact(boss||enemyCards.find(Boolean),p,level===3?2:1.45);
   v1892ScreenShake();pcLog(`EVOLUCIÓN DE ARENA — ${enemyDisplayName}: ${v.label}.`,'effect');
 }
}
function pcEquipmentCinematic(side,i,kind,meta){
 const g=board?.[side]?.[i];if(!g||!meta)return;
 const col=kind==='weapon'?0xffb24d:kind==='armor'?0x65b8ff:0xc875ff;
 pcEvolutionImpact({tags:[kind==='weapon'?'fire':kind==='armor'?'ice':'divine']},g.position,1.15);
 const ring=new THREE.Mesh(new THREE.TorusGeometry(1.05,.035,12,64),new THREE.MeshBasicMaterial({color:col,transparent:true,opacity:.9,depthWrite:false,blending:THREE.AdditiveBlending}));
 ring.rotation.x=Math.PI/2;ring.position.copy(g.position);ring.position.y+=.18;scene.add(ring);pcElementSystems.push(ring);ring.userData={kind:'EQUIP_RING',birth:performance.now(),life:900,spin:4.2,opacity:.9};
}
...
async function pcUltimateCinematic(side,i,c,attackName='ULTIMATE NÉMESIS'){
 if(!c)return;const d=document.createElement('div');d.className='pc-ultimate-cinematic';
 d.innerHTML=`<div class="pc-ultimate-storm"></div><section><small>ULTIMATE</small><img src="${c.img}" alt="${esc(c.name)}"><b>${esc(c.name)}</b><strong>${esc(attackName)}</strong></section>`;
 document.body.appendChild(d);d.offsetWidth;d.classList.add('show');const g=board?.[side]?.[i];
 if(g){pcEvolutionImpact(c,g.position,2.1);v1892ScreenShake()}sfx('ultimate');await wait(720);d.classList.add('strike');await wait(520);d.classList.add('out');await wait(160);d.remove()
}
function pcAttackSignature(c,meta={}){
 const e=pcEvolutionElement(c),name=meta.attackName||c?.attackName||c?.abilityName||'ATAQUE NÉMESIS';
 return {element:e,name,ultimate:PC_ULTIMATE_IDS.has(c?.id)||c?.rarity==='nemesis'||c?.type==='fusion'}
}
