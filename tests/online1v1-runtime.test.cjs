...
(async()=>{
 ...
 if(abilitySource>=0){
  ...
  if(ability.status===200){const log=ability.payload.room.duel.log||[];assert.ok(log.length>0);assert.ok(log.some(e=>['ABILITY','MGR_ABILITY','IDR_ABILITY','IDR_TRANSFORM','IDR_FUSION','ZEUS_NEGATE','DAMAGE','BUFF','DEBUFF','HEAL','RESURRECT','NEGATE','EFFECT','ARCANE_SEAL','ASCENSION','PROTECT'].includes(e.type))||log.some(e=>String(e.text||e.message||'').length>0));}
 }
