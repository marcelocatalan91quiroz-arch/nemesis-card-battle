...
assert(!index.includes('<script src="js/owner-auth.js"'),'Owner Auth no debe competir con el arranque.');
assert(!index.includes('<script src="js/online1v1.js"'),'Online no debe competir con el arranque.');
assert(game.includes('function nemesisScheduleOptionalModules()'),'Los módulos opcionales deben cargarse después del menú.');
assert(game.indexOf('title();') < game.indexOf('nemesisScheduleOptionalModules();'),'El menú debe dibujarse antes de Online/Auth.');
