...
const EXPECTED={DUEL_MASTER:20,MAGO_ROJO:20,IMPERIO_DRAGON:20,CABALLEROS_SUBMUNDO:20,OLIMPO:11,GUARDIAN:10,DRAGON_OJO:12,IRA_DE_RA:15,CABALLERO_ALMAS:10,REY_ESPECTRAL:10,DIOS_FANTASMA:10,ARES:12,HADES:12};
...
// El Guardián actual declara 10 cartas reales y todas deben existir en el pool.
const guardianDeclared=ids('GUARDIAN_BOSS_CARD_IDS');
const guardianMissing=guardianDeclared.filter(id=>!pool.has(id));
if(guardianMissing.length)throw Error('Arena Global: GUARDIAN IDs reales faltantes '+guardianMissing.join(','));
