...
const allArrays=['BASE_CARDS','NEW_CARDS','PLAYER_EXCLUSIVE_CARDS','DIVINE_PLAYER_CARDS','DRAGON_OJO_CARDS','ANCESTRAL_CARDS','SPECTRAL_CARDS','REY_ESPECTRAL_CARDS','DIOS_FANTASMA_CARDS','ARES_CARDS_1_5','HADES_CARDS','OLIMPO_CARDS','CABALLEROS_SUBMUNDO_CARDS','MAGO_ROJO_CARDS','IMPERIO_DRAGON_CARDS'];
...
for(const [n,d] of Object.entries(defs))if(!d.cards.length)throw Error('Arena Global: '+n+' sin cartas');
...
const report={simulator:'NEMESIS_ARENA_GLOBAL_V1',method:'seeded deterministic Monte Carlo + strategic effect heuristics',gamesPerMatchup:N,totalGames:matchups.length*N,decks:names.length,seed,ranking,matchups,topCards:cardRanking,note:'Laboratorio de balance: aproxima efectos complejos mediante heurísticas; no reemplaza pruebas runtime/humanas.'};
